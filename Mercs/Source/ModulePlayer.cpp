...
Update_Status ModulePlayer::Update()
{
	...
	{
		if (App->input->keys[SDL_SCANCODE_LEFT] == Key_State::KEY_REPEAT || pad.left_x < 0.0f)
		{
		...
		if (App->input->keys[SDL_SCANCODE_RIGHT] == Key_State::KEY_REPEAT || pad.left_x > 0.0f)
		{
			App->player->position.x = App->player->position.x + speedX;
			//App->render->camera.x += cameraSpeedX;
		...
		if (App->input->keys[SDL_SCANCODE_DOWN] == Key_State::KEY_REPEAT || pad.left_y > 0.0f)
		{
		...
		if ((App->input->keys[SDL_SCANCODE_LEFT] == Key_State::KEY_REPEAT && App->input->keys[SDL_SCANCODE_DOWN] == Key_State::KEY_REPEAT)
			|| (pad.left_x < 0.0f && pad.left_y > 0.0f))
		{
		...
		if (App->input->keys[SDL_SCANCODE_UP] == Key_State::KEY_REPEAT || pad.left_y < 0.0f)
		{
			App->player->position.y = App->player->position.y - speedY;
			//App->render->camera.y -= cameraSpeedY;
